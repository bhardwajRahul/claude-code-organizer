...
如果你係由舊名 **Claude Code Organizer**、舊 search term `claude-code-organizer`，或者冇 hyphen 嘅寫法 **Cross Code Organizer** 搵到嚟：呢個係同一個 project 嘅新定位。而家嘅 project name 同 current package/repo slug 係 **Cross-Code Organizer** / `cross-code-organizer`，仍然深度支援 **Claude Code**、**Codex CLI** 同 **MCP**，但已經唔再只係 Claude-only 工具。
...
> **私隱：** 匿名 metrics 預設關閉。只有你自己開啟後，CCO 先會傳一個會去重嘅每月訊號：UTC 月份、每月輪換嘅匿名 ID、CCO 版本同 harness ID。唔會傳路徑、名稱、prompt、檔案內容、session 或憑證。詳情見 [PRIVACY.md](PRIVACY.md)。

![Cross-Code Organizer (CCO) Demo](docs/demo.gif)

<sub>明確 opt-in telemetry | 唔上傳 config 或內容 | Demo 由 AI 用 [Pagecast](https://github.com/mcpware/pagecast) 錄製</sub>
...
**掃** → 所有記憶、技能、MCP server、規則、指令、agent、hook、plugin、plan、session，跨所有 project 同 harness，一個畫面睇晒。

**搵** → Show Effective 顯示 Claude Code 實際喺每個 project load 咗啲乜；Codex view 顯示 `~/.codex`、AGENTS、profiles、sessions 同 runtime 檔。Context Budget 話你知邊度食緊 token。Security Scanner 話你知邊個 server 有毒。
...
| 技能（含 bundle/plugin 來源識別） | 得 | 得 | 得 | Global + Project |
...
| Session（含 distill + image trim） | 得 | — | 得 | 淨係 Project |
