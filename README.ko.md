...
검색용 rename alias: 이 프로젝트는 예전에 **Claude Code Organizer** / `claude-code-organizer`로 불렸어요. 지금 이름은 **Cross Code Organizer** / `cross-code-organizer`예요. Claude Code, Codex CLI, MCP 설정 정리를 찾는 사람을 위한 legacy/search term이고, install이나 repo link는 현재 `cross-code-organizer`를 써야 해요.
